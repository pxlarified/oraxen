...
public class BlockStateManager {
    ...
    public void initializeBlockState(@NotNull Block block, @NotNull CustomBlock customBlock) {
        ...
        for (BlockProperty property : customBlock.getProperties().getProperties().values()) {
            state.put(property.getName(), property.getDefaultValue());
        }
    ...
    public void updateBlockProperty(@NotNull Block block, @NotNull String propertyName, @NotNull String value) {
        ...
        final Object typedValue = property.stringToValue(value);
        if (!property.isValidValue(typedValue)) {
            return;
        }
        ...
        newState.put(propertyName, property.valueToString(typedValue));
    ...
    private CustomBlock getCustomBlockAtLocation(@NotNull Block block) {
        for (CustomBlock customBlock : blockManager.getLoadedBlocks()) {
            if (block.getType() == customBlock.getSettings().getMaterial()) {
                if (isBlockAtLocation(block, customBlock)) {
                    return customBlock;
                }
            }
        }
    ...
    private boolean isBlockAtLocation(@NotNull Block block, @NotNull CustomBlock customBlock) {
        return isBlockAtLocationMatching(block, customBlock);
    }
